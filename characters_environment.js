...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Click on the Start button to play the game. ", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
  ...
  if(gameConfig.status==='gameover'){
    ...
    textSize(15);
    text("Press START button to Restart", gameConfig.screenX/2, gameConfig.screenY/2+135);
    textSize(40);
...
function manualControl(character){
  ...
  if(character.live){
    ...
    if(!noseX<375&&!noseX>375&&!keyDown(control.up)){ 
